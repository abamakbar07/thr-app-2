...
export default async function RoomsPage() {
  ...
  console.log("Full session object:", session?.user);
  ...
  try {
    // Use ObjectId for querying if possible, otherwise fallback to string
    let createdByValue;
    try {
      createdByValue = new mongoose.Types.ObjectId(session.user.id);
    } catch (error) {
      // If conversion fails, use the string value directly
      createdByValue = session.user.id;
    }

    // Fetch rooms created by the current user
    const rooms = await Room.find({ createdBy: createdByValue }).sort({ createdAt: -1 }).lean();
    console.log('Query used:', { createdBy: createdByValue });
    
    // Convert MongoDB documents to plain objects with proper typing
    const roomsData: RoomType[] = rooms.map((room: any) => ({
      _id: room._id.toString(),
      name: room.name as string,
      description: room.description as string,
      accessCode: room.accessCode as string,
      isActive: room.isActive as boolean,
      startTime: room.startTime as Date,
      endTime: room.endTime as Date,
      createdBy: typeof room.createdBy === 'object' ? room.createdBy.toString() : room.createdBy,
    ...
    return (
      <div>
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-medium text-gray-800">Game Rooms</h1>
          <Link 
    ...
    return (
      <div className="text-center py-12 bg-white shadow rounded-lg">
        <h3 className="mt-2 text-lg font-medium text-gray-900">Error Loading Rooms</h3>
        <p className="mt-1 text-sm text-gray-500">There was a problem loading your game rooms. Please try again later.</p>
      </div>
    );
  }
} 